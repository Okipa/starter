...
@section('template')
    <h1>
        ...
        @if ($dynamicPageBlock)
            @lang('admin.title.orphan.edit', ['entity' => __('dynamic-pages.entities.dynamicPageBlocks'), 'detail' => __(config('dynamic-pages.blocks.text_image.name')) ])
        @else
   ...
   <div class="card">
       ...
       <div class="card-body">
           <form action="{{ $dynamicPageBlock ?
                            route('dynamicPageBlock.text_image.update', [ $dynamicPage, $dynamicPageBlock, 'blockId' => request()->query('blockId') ]) :
                            route('dynamicPageBlock.text_image.store', [ $dynamicPage, 'blockId' => request()->query('blockId') ]) }}"
                 method="POST"
